...
describe('Cost Calculator', () => {
    describe('calculateUserCost', () => {
        test('calculates cost with default price', () => {
            expect(calculateUserCost(10)).toBe(50.00);
            expect(calculateUserCost(0)).toBe(0);
            expect(calculateUserCost(1)).toBe(5.00);
        });

        test('calculates cost with custom price', () => {
            expect(calculateUserCost(10, 6.50)).toBe(65.00);
            expect(calculateUserCost(5, 4.00)).toBe(20.00);
        });

        test('handles edge cases', () => {
            expect(calculateUserCost(-5, 5.00)).toBe(0); // Negative beers
            expect(calculateUserCost(10, 0)).toBe(0); // Zero price
            expect(calculateUserCost(10, -5)).toBe(0); // Negative price
            expect(calculateUserCost(0, 10.00)).toBe(0); // Zero beers
        });

        test('uses default price when eventPrice is undefined', () => {
            expect(calculateUserCost(10, undefined)).toBe(50.00);
        });
    });
    ...
    describe('calculateRoundCosts', () => {
        test('calculates costs for multiple users with default price', () => {
            ...
            expect(costs.get('user1')).toBe(50.00);
            expect(costs.get('user2')).toBe(25.00);
        ...
        test('calculates costs for multiple users with custom price', () => {
            ...
            const costs = calculateRoundCosts(beerCounts, 7.00);
            
            expect(costs.get('user1')).toBe(70.00);
            expect(costs.get('user2')).toBe(35.00);
        });
    ...
    describe('calculateTotalBill', () => {
        test('calculates total bill with default price', () => {
            expect(calculateTotalBill(15)).toBe(75.00);
            expect(calculateTotalBill(0)).toBe(0);
        });

        test('calculates total bill with custom price', () => {
            expect(calculateTotalBill(20, 6.00)).toBe(120.00);
            expect(calculateTotalBill(10, 4.50)).toBe(45.00);
        });
    });

    describe('formatCost', () => {
        test('formats cost with CHF symbol by default', () => {
            expect(formatCost(50.00)).toBe('50.00 CHF');
        ...
        test('formats cost without CHF symbol when specified', () => {
            expect(formatCost(50.00, false)).toBe('50.00');
    ...
    describe('DEFAULT_BEER_PRICE', () => {
        test('is set to 5.00', () => {
            expect(DEFAULT_BEER_PRICE).toBe(5.00);
        });
    });
});
