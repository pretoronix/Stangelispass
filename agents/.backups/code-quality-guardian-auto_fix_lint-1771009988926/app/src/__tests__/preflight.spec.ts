import { assertSupabaseConfigured, warnIfWebUnsupported } from '@/utils/preflight';
...
describe('preflight', () => {
    ...
    test('assertSupabaseConfigured returns true when present', () => {
        const ok = assertSupabaseConfigured({ url: 'https://example.supabase.co', anonKey: 'anon' });
        expect(ok).toBe(true);
    });
    ...
    test('assertSupabaseConfigured reads from process.env', () => {
        ...
        const originalKey = (global as any).process.env.EXPO_PUBLIC_SUPABASE_ANON_KEY;
        (global as any).process.env.EXPO_PUBLIC_SUPABASE_URL = 'https://env.supabase.co';
    ...
    test('assertSupabaseConfigured throws with scope/action in production', () => {
        ...
        try {
            expect(() => assertSupabaseConfigured({ url: '', anonKey: '' })).toThrow('preflight:supabase_config');
            try {
