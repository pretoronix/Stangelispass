...
export class RollbackManager {
    backupDir;
    constructor(backupDir = path.join(process.cwd(), 'agents', '.backups')) {
     ...
     */
    async createBackup(files, agentName, actionName) {
     ...
     */
    async restoreBackup(backupInfo, agentName, actionName) {
        ...
        for (const [originalPath, backupPath] of backupInfo.files.entries()) {
            try {
                ...
                logger.error(`Failed to restore ${originalPath}`, { error: error.message }, agentName, actionName);
                failedCount++;
            }
        }
     ...
     */
    async getAffectedFiles(pattern) {
