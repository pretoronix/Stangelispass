...
CREATE TABLE IF NOT EXISTS users (
  ...
  weight_kg FLOAT DEFAULT 80.0,
  gender TEXT DEFAULT 'neutral', -- 'male', 'female', 'neutral'
  notification_prefs JSONB DEFAULT '{"leader_change": true, "milestones": [5, 10, 20]}'::jsonb,
...
BEGIN
    ...
    FOR r IN (SELECT name FROM users GROUP BY name HAVING COUNT(*) > 1) LOOP
        ...
        IF EXISTS (SELECT FROM pg_tables WHERE schemaname = 'public' AND tablename = 'event_memberships') THEN
            UPDATE event_memberships SET user_id = master_id WHERE user_id IN (SELECT id FROM users WHERE name = r.name AND id <> master_id);
            UPDATE event_memberships SET invited_by = master_id WHERE invited_by IN (SELECT id FROM users WHERE name = r.name AND id <> master_id);
        END IF;
...
-- 3. Event Memberships (Event-scoped permissions)
...
-- Ensure event creator always becomes owner in membership table
CREATE OR REPLACE FUNCTION ensure_event_owner_membership()
RETURNS trigger
LANGUAGE plpgsql
AS $$
BEGIN
...
DROP TRIGGER IF EXISTS ensure_event_owner_membership_trigger ON events;
CREATE TRIGGER ensure_event_owner_membership_trigger
AFTER INSERT ON events
FOR EACH ROW
EXECUTE PROCEDURE ensure_event_owner_membership();

-- Backfill owner role for existing events
INSERT INTO event_memberships(event_id, user_id, role, status, invited_by, joined_at)
...
ALTER TABLE users ADD COLUMN IF NOT EXISTS weight_kg FLOAT DEFAULT 80.0;
ALTER TABLE users ADD COLUMN IF NOT EXISTS gender TEXT DEFAULT 'neutral';
ALTER TABLE users ADD COLUMN IF NOT EXISTS notification_prefs JSONB DEFAULT '{"leader_change": true, "milestones": [5, 10, 20]}'::jsonb;
...
CREATE INDEX IF NOT EXISTS idx_event_memberships_event_id ON event_memberships(event_id);
CREATE INDEX IF NOT EXISTS idx_event_memberships_user_id ON event_memberships(user_id);
CREATE INDEX IF NOT EXISTS idx_event_memberships_role ON event_memberships(role);
...
BEGIN
    ...
    END IF;
    IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE policyname = 'Public Access' AND tablename = 'event_memberships') THEN
        CREATE POLICY "Public Access" ON event_memberships FOR ALL USING (true) WITH CHECK (true);
    END IF;
