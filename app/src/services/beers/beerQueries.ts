...
export const getBeerCountByUser = async (
    eventId?: string
): Promise<{ userId: string; name: string; count: number; isAdmin: boolean }[]> => {
    ...
    const safeUsers = (users as Array<{ id: string; name: string; is_admin: boolean }> | null) || [];
    if (beersError) {
...
export const getUserAchievements = async (userId: string): Promise<Achievement[]> => {
    ...
    if (error) {
        if (isMissingTableError(error)) {
            logMissingTable('achievements', 'Returning empty achievements (expected)');
            return [];
        }
        throw error;
    }
