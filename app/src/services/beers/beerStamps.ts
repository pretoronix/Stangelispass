...
export const createBeerStamp = async (
  ...
  const { data, error } = await (supabase.from("beer_stamps") as any)
    .insert({
      user_id: userId,
      event_id: eventId,
      issued_by: issuedBy,
...
export const redeemBeerStamp = async (
  ...
  const { data: stamp, error: fetchError } = await (
    supabase.from("beer_stamps") as any
  )
    .select("*")
    .eq("id", stampId)
    .maybeSingle();
  ...
  if (fetchError) {
    if (isMissingTableError(fetchError)) {
      return {
        ok: false,
        reason: "stamps_unavailable",
        beer: null,
        newBadges: [] as BadgeType[],
      };
    }
    throw fetchError;
  }
  ...
  try {
    ...
    await (supabase.from("beer_stamps") as any)
