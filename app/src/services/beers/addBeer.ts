...
export const addBeer = async (
  ...
  const { data: newBeer, error } = await (supabase.from("beers") as any)
    .insert({
      user_id: userId,
      added_by: addedBy,
      event_id: eventId,
  ...
  if (!newBeer || !newBeer.id) {
    throw new Error(
      "Failed to create beer: Supabase returned no data. Check database connection and RLS policies.",
    );
  }
  ...
  const safeNewBeer =
    (newBeer as Beer) ||
    ({ created_at: new Date().toISOString(), user_id: userId } as Beer);
