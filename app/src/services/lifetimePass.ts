...
export const listLifetimePassCodes = async (): Promise<LifetimePassCode[]> => {
    const { data, error } = await (supabase
        .from('lifetime_pass_codes') as any)
        .select('*')
        .order('created_at', { ascending: false });

    if (error) {
        if (isMissingTableError(error)) {
            logExpected('table `lifetime_pass_codes` not found. Returning empty codes list.', 'lifetime_pass');
            return [];
        }
        throw error;
    }
...
export const createLifetimePassCode = async (createdBy: string): Promise<LifetimePassCode | null> => {
    for (let attempt = 0; attempt < 3; attempt += 1) {
        ...
        if (error) {
            if (isMissingTableError(error)) {
                logExpected('table `lifetime_pass_codes` not found. Cannot create code.', 'lifetime_pass');
                return null;
            }
...
export const redeemLifetimePassCode = async (
    ...
    if (error) {
        if (isMissingTableError(error)) {
            logExpected('table `lifetime_pass_codes` not found. Cannot redeem code.', 'lifetime_pass');
