...
const loadModule = async (opts: {
  ...
  users?: { id: string; notification_prefs?: any }[];
  insertError?: any;
}) => {
  jest.resetModules();
  ...
  jest.doMock("expo-notifications", () => ({
    getPermissionsAsync: jest.fn(async () => ({
      status: opts.permissions?.existing ?? "granted",
    })),
    requestPermissionsAsync: jest.fn(async () => ({
      status: opts.permissions?.requested ?? "granted",
    })),
    getExpoPushTokenAsync: jest.fn(async () => ({
      data: opts.token ?? "ExponentPushToken[test]",
    })),
  ...
  const fromCalls: { table: string; args?: any }[] = [];
      ...
      if (table === "users") {
        ...
        const builder: any = {
          ...
          then: (resolve: any, reject: any) =>
            Promise.resolve(usersResult).then(resolve, reject),
        };
        return builder;
      }
...
describe("services/notifications", () => {
  ...
  it("registerForPushNotificationsAsync returns token and upserts device_tokens", async () => {
    ...
    expect(
      (supabase.from as unknown as jest.Mock).mock.calls.some(
        (c) => c[0] === "device_tokens",
      ),
    ).toBe(true);
  });
  ...
  it("sendAdminBroadcast enqueues notifications for opted-in members only", async () => {
    ...
    const insertCall = fromCalls.find(
      (c) => c.table === "notifications.insert",
    );
  ...
  it("enqueueNewRoundNotifications enqueues for opted-in users excluding sender", async () => {
    ...
    const res = await mod.enqueueNewRoundNotifications(
      "evt1",
      "Event Name",
      "u1",
    );
