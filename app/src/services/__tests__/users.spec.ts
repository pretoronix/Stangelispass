...
jest.mock("@/services/client", () => {
  ...
  chain.order = jest.fn(async () => ({
    data: [{ id: "u1", name: "Alice", is_admin: true }],
    error: null,
  }));
  ...
  chain.single = jest.fn(async () => ({
    data: { id: "u2", name: "Bob", is_admin: true },
    error: null,
  }));
...
describe("services/users", () => {
  ...
  test("getUsers returns empty list on missing table", async () => {
    ...
    client.__chain.order.mockResolvedValueOnce({
      data: null,
      error: { code: "PGRST205" },
    });
  ...
  test("addUser returns null on missing table", async () => {
    const client: any = require("@/services/client");
    client.__chain.single.mockResolvedValueOnce({
      data: null,
      error: { code: "PGRST205" },
    });
