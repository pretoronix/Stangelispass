...
const fetchLeaderLastBeerAt = async (eventId: string, leaderId: string) => {
  const { data, error } = await (supabase.from("beers") as any)
    .select("created_at")
    .eq("event_id", eventId)
    .eq("user_id", leaderId)
...
export const createLeaderEventSnapshot = async (
  eventId: string,
): Promise<EventLeaderSnapshot | null> => {
  try {
    ...
    const leaderLastBeerAt =
      leader.last_beer_at ||
      (await fetchLeaderLastBeerAt(eventId, leader.user_id));

    const { data, error } = await (
      supabase.from("event_leader_snapshots") as any
    )
      .insert({
        event_id: eventId,
    ...
    if (error) {
      if (isMissingTableError(error)) {
        logExpected(
          "table `event_leader_snapshots` not found. Snapshot skipped.",
          "Events",
        );
        return null;
      }
