...
const setup = async (opts: {
  ...
  leaderLastBeer?: { data: Array<{ created_at: string }>; error: any };
  insertError?: any;
  insertData?: any;
}) => {
  jest.resetModules();
          ...
          select: jest.fn((cols: string) => {
            if (cols === 'created_at') {
              return {
                ...
                limit: jest.fn(async () => opts.leaderLastBeer ?? { data: [], error: null }),
              };
            }
            // buildLastBeerByUser
            return {
              eq: jest.fn(async () => opts.beersByUser ?? { data: [], error: null }),
            };
          }),
        };
      }
...
describe('createLeaderEventSnapshot', () => {
  ...
  it('uses event_game_stats leaderboard when available', async () => {
    ...
    await expect(mod.createLeaderEventSnapshot('evt1')).resolves.toEqual({ id: 'snap_1' });
  ...
  it('fetches leader last beer time when missing from stats', async () => {
    const { mod, insertPayloads } = await setup({
      ...
      leaderLastBeer: { data: [{ created_at: '2020-01-01T01:23:00.000Z' }], error: null },
      insertData: { id: 'snap_2' },
    });

    await expect(mod.createLeaderEventSnapshot('evt1')).resolves.toEqual({ id: 'snap_2' });
    expect(insertPayloads[0].leader_last_beer_at).toBe('2020-01-01T01:23:00.000Z');
  });
  ...
  it('tolerates missing beers table when fetching leader last beer time', async () => {
    const { mod, insertPayloads } = await setup({
      ...
      leaderLastBeer: { data: [], error: { code: '42P01', message: 'missing table' } },
      insertData: { id: 'snap_3' },
    });

    await expect(mod.createLeaderEventSnapshot('evt1')).resolves.toEqual({ id: 'snap_3' });
  ...
  it('builds leaderboard from beers when event_game_stats is missing', async () => {
    ...
    await expect(mod.createLeaderEventSnapshot('evt1')).resolves.toEqual({ id: 'snap_4' });
    expect(insertPayloads[0]).toMatchObject({ leader_id: 'u1', leader_beer_count: 2 });
  });
