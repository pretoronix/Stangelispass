...
import { createLeaderEventSnapshot } from "./leaderSnapshots";
...
export async function startEventInSupabase({
  ...
  if (passType !== "free") {
    try {
      ...
      reportError(countErr as Error, {
        scope: "app_provider",
        action: "resolve_pass_type",
      });
    }
  }
...
export async function closeEventInSupabase(activeEvent: Event): Promise<void> {
  ...
  if (!countError && counts && counts.length > 0) {
    ...
    if (winnerCount > 0) {
      await (supabase.from("wall_of_fame") as any).insert({
  ...
  try {
    ...
    reportError(snapshotError as Error, {
      scope: "app_provider",
      action: "leader_snapshot",
    });
  }
