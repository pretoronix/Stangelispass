...
export async function checkAndClearIfOversized(maxSizeMB: number = 5): Promise<boolean> {
    try {
        ...
        if (stats.sizeKB > maxSizeKB) {
            console.warn(`[CacheManager] Cache size (${stats.sizeKB}KB) exceeds limit (${maxSizeKB}KB), clearing...`);
            await clearCache();
            return true;
        }
...
export async function getAllCacheKeys(): Promise<string[]> {
    try {
        ...
        console.error('[CacheManager] Error getting cache keys:', error);
        return [];
    }
}
...
export async function clearAllCacheVersions(): Promise<void> {
    try {
        ...
        console.error('[CacheManager] Error clearing all cache versions:', error);
        throw error;
    }
}
