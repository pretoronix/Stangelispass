...
export function useAddBeer() {
    ...
    return useMutation({
        ...
        onMutate: async (variables) => {
            ...
            await queryClient.cancelQueries({ queryKey: QUERY_KEYS.beers(variables.eventId) });
            await queryClient.cancelQueries({ queryKey: QUERY_KEYS.beerCounts(variables.eventId) });
            
            // Snapshot previous values
            const previousBeers = queryClient.getQueryData(QUERY_KEYS.beers(variables.eventId));
            const previousCounts = queryClient.getQueryData(QUERY_KEYS.beerCounts(variables.eventId));
            ...
            queryClient.setQueryData<any[]>(
                ...
                (old = []) => {
                    const userExists = old.some((u: any) => u.userId === variables.userId);
                    if (userExists) {
...
export function useRemoveBeer() {
    ...
    return useMutation({
        ...
        onMutate: async (beerId) => {
            ...
            const previousCounts = queryClient.getQueriesData({ queryKey: ['beer-counts'] });
            
            // Optimistically remove from all beer lists
            queryClient.setQueriesData<any[]>(
                { queryKey: ['beers'] },
                (old = []) => old.filter((beer: any) => beer.id !== beerId)
            );
