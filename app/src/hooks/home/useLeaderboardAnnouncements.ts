...
export function useLeaderboardAnnouncements({
  ...
  const [leaderAnnouncement, setLeaderAnnouncement] = useState<string | null>(
    null,
  );
  const [streakAnnouncement, setStreakAnnouncement] = useState<string | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    if (
      nextStreak > (streakRef.current || 0) &&
      isStreakMilestone(nextStreak)
    ) {
