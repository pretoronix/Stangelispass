...
describe('useExportData', () => {
  ...
  it('alerts when no active event provided', async () => {
    ...
    expect(alertSpy).toHaveBeenCalledWith('Export', 'No active round to export.');
  });
  ...
  it('alerts when there are no beers for the event', async () => {
    ...
    expect(alertSpy).toHaveBeenCalledWith('Export', 'No beers logged for this event yet.');
  });
  ...
  it('exports via download on web', async () => {
    ...
    (global as any).document = {
      ...
      createElement: jest.fn(() => ({ click, set href(v: string) {}, set download(v: string) {} })),
    };
    ...
    expect(alertSpy).toHaveBeenCalledWith('Export', 'CSV downloaded successfully!');
  });
  ...
  it('reports and alerts on native filesystem failures', async () => {
    ...
    (FileSystem.writeAsStringAsync as jest.Mock).mockRejectedValueOnce(new Error('fs'));
