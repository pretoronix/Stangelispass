...
describe('useScanHandler', () => {
  ...
  it('alerts on unknown payload', async () => {
    ...
    const { handleScan } = useScanHandler(null, null, { canManageLogs: false }, openNamePrompt, setScanning, refresh);
    await handleScan('x');
  ...
  it('join_event prompts for name when user is missing', async () => {
    (parseScanPayload as jest.Mock).mockReturnValueOnce({ type: 'join_event', eventName: 'Round', eventId: 'e1' });
    const setScanning = jest.fn();
    const refresh = jest.fn();
    const openNamePrompt = jest.fn();
  ...
  it('join_event calls joinEvent when user exists and reports join errors', async () => {
    ...
    const { handleScan } = useScanHandler({ id: 'u1' }, null, { canManageLogs: false }, openNamePrompt, setScanning, refresh);
    await handleScan('x');
  ...
  it('stamp_redeem blocks when user is missing', async () => {
    (parseScanPayload as jest.Mock).mockReturnValueOnce({ type: 'stamp_redeem', stampId: 's1' });

    const { handleScan } = useScanHandler(null, null, { canManageLogs: false }, jest.fn(), jest.fn(), jest.fn());
    await handleScan('x');
  ...
  it('stamp_redeem alerts on invalid stamp and completes on success', async () => {
    ...
    const setScanning = jest.fn();
    const refresh = jest.fn();
    const { handleScan } = useScanHandler({ id: 'u1' }, null, { canManageLogs: false }, jest.fn(), setScanning, refresh);
  ...
  it('logs beer via scan when authorized', async () => {
    (parseScanPayload as jest.Mock).mockReturnValueOnce({ type: 'beer', userId: 'u2', eventId: 'e1' });
