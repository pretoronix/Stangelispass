...
export const useEventManagement = ({
    ...
    }, [currentUser, eventPermissions, newEventName, newEventPassType, startEvent]);
    ...
    const handleEventRoleChange = useCallback(async (member: EventMembership, role: EventRole) => {
        ...
        try {
            ...
            reportError(e as Error, {
                ...
                userId: currentUser.id,
                metadata: { eventId: activeEvent.id, memberId: member.user_id, newRole: role },
            });
            Alert.alert('Error', 'Could not update event role.');
        }
    }, [activeEvent, currentUser, refreshEventMembers]);

    const handleAddEventMember = useCallback(async (userId: string, role: EventRole) => {
        if (!activeEvent || !currentUser) return;
        try {
    ...
    const handleRemoveEventMember = useCallback(async (member: EventMembership) => {
        if (!activeEvent) return;
                    ...
                    onPress: async () => {
                        try {
                            ...
                            reportError(e as Error, {
                                ...
                                metadata: { eventId: activeEvent.id, memberId: member.user_id },
                            });
