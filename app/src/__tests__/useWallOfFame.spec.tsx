...
describe("useUserToasts", () => {
  ...
  it("should fetch user toasts", async () => {
    ...
    (wallOfFameService.getUserToasts as jest.Mock).mockResolvedValue(
      mockToasts,
    );
...
describe("useBeerClink", () => {
  ...
  it("should add toast when not already toasted", async () => {
    ...
    await waitFor(() => {
      expect(wallOfFameService.addToast).toHaveBeenCalledWith(
        "wall1",
        "user123",
      );
    });
  ...
  it("should remove toast when already toasted", async () => {
    ...
    await waitFor(() => {
      expect(wallOfFameService.removeToast).toHaveBeenCalledWith(
        "wall1",
        "user123",
      );
    });
