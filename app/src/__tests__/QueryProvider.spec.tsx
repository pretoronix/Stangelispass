...
describe('QueryProvider', () => {
    ...
    it('clears old cache versions on mount', async () => {
        ...
        AsyncStorage.getAllKeys.mockResolvedValueOnce([CURRENT_KEY, 'STANGELISPASS_QUERY_CACHE_0.9.0_v1']);
        ...
        await waitFor(() => {
            expect(AsyncStorage.multiRemove).toHaveBeenCalledWith(['STANGELISPASS_QUERY_CACHE_0.9.0_v1']);
        });
    });
    ...
    it('configures shouldDehydrateQuery to skip sensitive keys and require success', async () => {
        ...
        const shouldDehydrateQuery = capturedPersistOptions.dehydrateOptions?.shouldDehydrateQuery;
        expect(typeof shouldDehydrateQuery).toBe('function');

        expect(
            shouldDehydrateQuery({ queryKey: ['device-token'], state: { status: 'success' } })
        ).toBe(false);
        expect(
