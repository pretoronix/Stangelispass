...
describe('getBeerCountByUser', () => {
  ...
  test('maps counts correctly when beers are present', async () => {
    ...
    expect(result[0]).toEqual({ userId: 'u1', name: 'Alice', count: 2, isAdmin: false });
    expect(result[1]).toEqual({ userId: 'u2', name: 'Bob', count: 1, isAdmin: true });
  });

  test('returns empty array when users query is empty', async () => {
    mockFrom.mockReturnValueOnce({ select: jest.fn().mockResolvedValue({ data: null, error: null }) } as any);
    mockFrom.mockReturnValueOnce({ select: jest.fn().mockResolvedValue({ data: [], error: null }) } as any);
    const result = await getBeerCountByUser();
  ...
  test('applies event filter when event id is provided', async () => {
    ...
    const eq = jest.fn().mockResolvedValue({ data: [{ user_id: 'u1' }], error: null });
