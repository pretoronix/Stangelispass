...
describe("appProviderLifecycle", () => {
  ...
  test("bootstrapAppProvider restores user and refreshes data", async () => {
    (readStoredUser as jest.Mock).mockResolvedValue(
      JSON.stringify({ id: "u1", name: "Test" }),
    );
    ...
    expect(setCurrentUserState).toHaveBeenCalledWith({
      id: "u1",
      name: "Test",
    });
