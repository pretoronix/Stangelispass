...
describe("Phase 2: Leaderboard & Achievements (Host Perspective)", () => {
  ...
  describe('1. Hat Trick - "The Most Disputed Achievement"', () => {
    ...
    test("Hat Trick: Different users should not interfere", () => {
      ...
      const u1b2 = createTestBeer(
        user1.id,
        event.id,
        new Date(baseTime.getTime() + 15 * 60 * 1000).toISOString(),
      );
      const u1b3 = createTestBeer(
        user1.id,
        event.id,
      ...
      const u2b2 = createTestBeer(
        user2.id,
        event.id,
        new Date(baseTime.getTime() + 60 * 60 * 1000).toISOString(),
      );
      const u2b3 = createTestBeer(
        user2.id,
        event.id,
        new Date(baseTime.getTime() + 120 * 60 * 1000).toISOString(),
      );
      db.addBeer(u2b1);
      db.addBeer(u2b2);
  ...
  describe('2. First Blood - "Who started the party?"', () => {
    ...
    test("Concurrent first beers: Only truly first gets badge", async () => {
      ...
      const secondBadges = checkAchievements(
        [beers[0] as Beer],
        beers[1] as Beer,
        1,
      );
      const thirdBadges = checkAchievements(
        [beers[0] as Beer, beers[1] as Beer],
        beers[2] as Beer,
        1,
      );
  ...
  describe('6. Leaderboard - "Who\'s Actually Winning?"', () => {
    test("20 users, 100+ beers: Leaderboard stays accurate", () => {
      ...
      for (let i = 0; i < leaderboard.length - 1; i++) {
        expect(leaderboard[i]!.count).toBeGreaterThanOrEqual(
          leaderboard[i + 1]!.count,
        );
      }
  ...
  describe('7. Badge Deduplication - "No Double Rewards"', () => {
    ...
    test("Hat Trick + Century Club + Weekend Warrior: All distinct", () => {
      ...
      const badges102 = checkAchievements(
        [beer1 as Beer, beer2 as Beer],
        beer3 as Beer,
        102,
      ); // Hat Trick beer
  ...
  describe("8. Real-World Achievement Scenario", () => {
    test("Full event: Multiple users unlock different badges", () => {
      ...
      const u2b1 = createTestBeer(
        users[1]!.id,
        event.id,
        new Date(baseTime.getTime() + 5 * 60 * 1000).toISOString(),
      );
      const u2b2 = createTestBeer(
        users[1]!.id,
        event.id,
        new Date(baseTime.getTime() + 25 * 60 * 1000).toISOString(),
      );
      const u2b3 = createTestBeer(
        users[1]!.id,
        event.id,
        new Date(baseTime.getTime() + 45 * 60 * 1000).toISOString(),
      );
      ...
      const u3b1 = createTestBeer(
        users[2]!.id,
        event.id,
        new Date("2026-02-14T03:00:00Z").toISOString(),
      );
      const beersBeforeU3 = db.getBeersForEvent(event.id);
      allBadges[users[2]!.id] = checkAchievements(
        beersBeforeU3 as Beer[],
        u3b1 as Beer,
        1,
      );
      db.addBeer(u3b1);
