...
## Stand 2026-08-16 (Ende)

| WS | Status | Ergebnis |
| --- | --- | --- |
| A — IAP entfernen | erledigt | `IAP_ENABLED = false`, `expo-in-app-purchases` raus, Kauf-UI ausgeblendet |
| B — App-Konfiguration | erledigt | `supportsTablet: false`, ATT raus, Photo-Add-Description ergänzt, Privacy-Manifest |
| C — Assets | erledigt | Icons/Splash/Favicon eigenständig, Quelle + `app/scripts/generate-assets.mjs` |
| D — Legal-Hosting | erledigt (Deploy offen) | `web/legal/` mit `/privacy`, `/terms`, `/imprint`, `/support` |
| E — Store-Listing | erledigt | `docs/deployment/store-listing-copy.md` final (Copy, Age-Rating, Nutrition Labels, Review Notes) |
| F — Compliance-Audit | erledigt | `docs/deployment/compliance-audit.md`; Alkohol-Hinweis in-App ergänzt |

Offen bleiben nur die manuellen Schritte unten (Support-E-Mail, Impressum, Vercel-Deploy,
`eas init`, Screenshots).
