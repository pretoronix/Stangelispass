...
- Migration file created (`app/supabase/migrations/20260212001334_create_comments.sql`)
- Integrated into beer log UI (History screen + comments components)
- Test coverage added (`app/src/__tests__/comments.spec.tsx`)
⚠️ **Deployment Note**:
- Run the migration on the target Supabase database before release.
...
✅ Implemented:
- Comments table migration (SQL file in repo)
- Full service layer (`app/src/services/comments.ts`)
- Query hooks with optimistic updates (`app/src/hooks/useCommentsQuery.ts`)
- Real-time subscriptions (`app/src/hooks/useRealtimeComments.ts`)
- UI components and integration in history/beer log flows
...
- ⚠️ Migration applied to production database (pending deploy)
- ✅ Integrated into actual beer log screens
- ✅ Unit/integration tests added (comments.spec.tsx)
